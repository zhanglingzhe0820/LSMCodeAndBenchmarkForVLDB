...
for delta in 50 10; do
  for mu in 4.0 5.0; do
    for sigma in 1.0 1.25 1.5 1.75 2.0; do
      ...
      cp lsm_write.properties ../iotdb-without_seperation_policy_no_deletion/apache-iotdb-0.12.1-SNAPSHOT-all-bin/conf/iotdb-engine.properties
...
for delta in 50 10; do
  for mu in 4.0 5.0; do
    for sigma in 1.5 1.75 2.0; do
      echo "${delta}_${mu}_${sigma}"
      cp tlsm_write.properties ../iotdb-with_seperation_policy_no_deletion/apache-iotdb-0.12.1-SNAPSHOT-all-bin/conf/iotdb-engine.properties
      ./run_tlsm.sh ${delta} ${mu} ${sigma}
      du -smh ../iotdb-with_seperation_policy_no_deletion/apache-iotdb-0.12.1-SNAPSHOT-all-bin/data
      cp lsm_compaction.properties ../iotdb-with_seperation_policy_no_deletion/apache-iotdb-0.12.1-SNAPSHOT-all-bin/conf/iotdb-engine.properties
